...
AI-powered create_report_ai orchestrator
- Understands freeform prompts and dynamically chooses tools
- Integrates describe_report_type to fetch canonical API field names, data types, and operator maps for a report type
- Resolves folders and submits the final reportMetadata via Analytics API

New AI tool: describe_report_type (internal to the agent)
- Input: { reportType?: string, objectName?: string }
- Output: { ok, reportType, fieldMap, fieldTypeMap, operatorMap, suggestions? }
- Internally queries /services/data/v39.0/analytics/report-types/{type} and builds a mapping using the same logic as buildReportFieldMap

Example prompt-to-execution
Prompt: "Create a summary report for Accounts showing Name and Industry grouped by Owner"
Flow the agent follows:
1) Calls describe_report_type with { objectName: "Account" } to get fieldMap/operatorMap
2) Maps columns: ["Name", "Industry"] -> ["NAME", "INDUSTRY"] and groupingDown by Owner -> e.g., "OWNER.NAME"
3) Builds reportMetadata with reportType { type: "AccountList", label: "Accounts" }, reportFormat: "SUMMARY", detailColumns, groupingsDown
4) Resolves folderId via lookup_report_folder (using provided folderName if any, else fallback)
5) Calls submit_salesforce_report with the final { reportMetadata }

You can invoke it like:

call_tool: salesforce_create_report_ai
arguments: { "prompt": "Create a summary report for Accounts showing Name and Industry grouped by Owner" }

The agent will output a compact JSON summary including the chosen reportType, mapped fields, folder, and the creation result.
